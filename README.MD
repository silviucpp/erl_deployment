...
This is a very tiny framework based on `ant` scripts and `fpm` which allows you to:
...
redis_pool.pool_size=10
redis_pool.host=127.0.0.1
redis_pool.port=6379

# main app

app.dependencies = lager,redis_pool
...
Also the `rebar.config` should include the following (important is `sys_config` and `vm_args`):
...
So basically a recommended project structure will be:
...
- `deb.systemd.start_script` - allows you to replace the bash script which systemd will use to start you app. defaults to `${project.deployment_scripts}/deb/start.sh`
